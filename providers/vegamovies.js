 ...
 * Generated: 2026-07-25T17:20:01.555Z
 */
...
var SITE_URL = "https://vegamovies.navy";
...
function parseDetailPage(html) {
  ...
  content.find("a[href]").each((_, el) => {
    ...
    if (href.includes("vegamovies") && !href.includes("redirect"))
      return;
    if (href.startsWith("#") || href.startsWith("javascript:"))
      return;
    if (href.includes("t.me/") || href.includes("facebook.com"))
      return;
    const isDownload = href.includes("drive.google.com") || href.includes("hubcloud") || href.includes("gdtot") || href.includes("filepress") || href.includes("links.") || href.includes("redirect") || text.toLowerCase().includes("download") || text.toLowerCase().includes("gdrive") || text.toLowerCase().includes("g-drive");
    if (!isDownload)
      return;
...
function extractStreams(tmdbId, mediaType, season, episode) {
  return __async(this, null, function* () {
    ...
    try {
      ...
      const query = mediaType === "movie" && tmdbInfo.year ? `${tmdbInfo.title} ${tmdbInfo.year}` : tmdbInfo.title;
      ...
      const html = yield fetchText2(bestUrl);
