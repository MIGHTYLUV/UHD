...
var olamovies_exports = {};
__export(olamovies_exports, {
  getStreams: () => getStreams
});
module.exports = __toCommonJS(olamovies_exports);
...
// src/olamovies/http.js
var BASE_URL = "https://v3.olamovies.mov";
function fetchHtml(url) {
  return __async(this, null, function* () {
    console.log(`[OlaMovies] Fetching ${url}`);
    return fetchText(url, __spreadProps(__spreadValues({}, DEFAULT_HEADERS), {
      Referer: BASE_URL
    }));
  });
}
...
// src/olamovies/extractor.js
function extractSearchResults(html, targetTitle, targetYear) {
...
function getStreams(tmdbId, mediaType, season, episode) {
  return __async(this, null, function* () {
    try {
      ...
      const detailHtml = yield fetchHtml(targetUrl);
      if (!detailHtml)
        return [];
